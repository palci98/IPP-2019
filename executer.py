...
#php test.php --directory=int-only --recursive --int-only --int-script=executer.py
    ...
    if instruction == "LT":
        ...
        if value2["type"]==value3["type"] == "int":
            ...
            for idx in range(min(len(value2["value"]), len(value3["value"]))):
                ordinal1, ordinal2 = ord(value2["value"][idx]), ord(value3["value"][idx])
...
# ---------------------------------------------------
    ...
    if instruction == "SETCHAR":
        ...
        if value["type"] == "int" and value2["type"]=="string":
...
# ---------------------------------------------------
    ...
    if instruction == "TYPE":
#           value={"type": "string","value": argument2["type"]
        if argument2["type"] == "var":
            value = GetValue(argument2["value"])
        else:
            value = argument2
